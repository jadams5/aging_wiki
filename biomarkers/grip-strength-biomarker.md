...
8. **Deliberate grip training decouples the marker from its substrate (Goodhart corruption)** — The mortality/morbidity associations were all established in cohorts where grip was an *incidental* measure: HGS predicted outcomes precisely because it was an integrative proxy of whole-body muscle, neural drive, nutrition, and disease burden (consistent with its **partial** — not full — MR causal status; see § Mendelian Randomization). The forearm/hand is a small, isolatable muscle group, and the dynamometer test carries a substantial neural/skill component, so an individual can raise measured HGS through grip- or hang-specific neural and forearm adaptations **without** improving the systemic substrate the number indexes. In anyone who trains grip directly (grippers, dead/active hangs, heavy carries, climbing), measured HGS therefore *overstates* systemic status relative to the cohort calibration, and cross-sectional comparison to EWGSOP2/normative cutoffs loses fidelity. This makes HGS **more gameable than performance biomarkers with no isolated shortcut** — VO₂max, whole-body/leg strength, gait speed, and DXA appendicular lean mass require genuine systemic adaptation to move and are preferable for a biological-age read in grip-trained individuals. A familiarization/learning effect (values rise on repeat testing independent of any biological change) compounds this on first retest. Within-person *trend* retains signal only when grip is not isolate-trained. #gap/no-mechanism
