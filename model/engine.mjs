...
/* --------------------------- time-profiles ----------------------------- */
// M1 (2026-06-14): every input channel is a typed TIME-PROFILE resolved to a per-age lane.
//   scalar (number|string)        → constant fill (back-compat: byte-identical to the old scalar path)
//   undefined | null              → popDefault fill (the invariant: default == population average)
//   {byAge:[[age,value],…], mode} → step (ZOH) | linear (flat-extrapolate ends) | pchip (LEGACY, untagged)
// `step` is for behavioural/exposure inputs + treatment windows (a value set at an age holds until the next
// entry; cessation is a hard cut; before the first entry → popDefault). `linear` is for personal-offset
// residuals (interpolate between draws, flat outside). UNTAGGED {byAge} keeps the legacy PCHIP reading so the
// pre-M1 former-smoker test is byte-identical; new histories MUST set mode explicitly. See
// model/timeline-history-import-design.md §2-3.
export function resolveProfile(p, AGES, popDefault, defaultMode = "pchip") {
  if (p === undefined || p === null) return AGES.map(() => popDefault);
  if (typeof p === "number" || typeof p === "string") return AGES.map(() => p);
  if (p && Array.isArray(p.byAge)) {
    if (p.byAge.length === 0) return AGES.map(() => popDefault);   // empty ⇒ no profile (not a throw / not 0)
    const mode = p.mode || defaultMode;
    if (mode !== "step" && mode !== "linear" && mode !== "pchip")
      throw new Error(`resolveProfile: unknown mode ${JSON.stringify(p.mode)} (expected step|linear|pchip)`);
    // Categorical (string-valued) profiles are STEP-ONLY — you cannot interpolate enum states.
    const categorical = typeof p.byAge[0][1] === "string";
    if (categorical && mode !== "step")
      throw new Error(`resolveProfile: categorical (string) profile requires mode:"step"`);
    // Defensive: COPY + sort ascending by age (don't mutate caller/MODEL arrays); collapse duplicate ages
    // (last wins) so numeric interpolation can't divide by a zero age-gap (PCHIP NaN / linear ∞).
    const sorted = [...p.byAge].sort((a, c) => a[0] - c[0]);
    if (mode === "step") return AGES.map((a) => stepLookup(sorted, a, popDefault));
    const dd = [];
    for (const kv of sorted) { if (dd.length && dd[dd.length - 1][0] === kv[0]) dd[dd.length - 1] = kv; else dd.push(kv); }
    if (mode === "linear") return AGES.map((a) => linearProfile(dd, a));
    return AGES.map((a) => interp(dd, a)); // legacy PCHIP (untagged byAge — back-compat)
  }
  return AGES.map(() => popDefault);
}
// Zero-order hold: carry the last entry whose age ≤ `age`; before the first entry → `before`. arr ascending.
function stepLookup(arr, age, before) {
  let v = before;
  for (let i = 0; i < arr.length; i++) { if (age >= arr[i][0]) v = arr[i][1]; else break; }
  return v;
}
// Piecewise-LINEAR with flat (constant) extrapolation past both ends — NOT PCHIP (no overshoot, no smoothing).
function linearProfile(arr, age) {
  const n = arr.length;
  if (n === 0) return 0;
  if (age <= arr[0][0]) return arr[0][1];
  if (age >= arr[n - 1][0]) return arr[n - 1][1];
  let i = 0; while (i < n - 1 && age > arr[i + 1][0]) i++;
  const f = (age - arr[i][0]) / (arr[i + 1][0] - arr[i][0]);
  return arr[i][1] + f * (arr[i + 1][1] - arr[i][1]);
}
// ONE canonical resolved-input map (id → per-age lane), shared by simulate()'s rate-terms + cause/all-cause
// edges AND mediators()'s mediator edges, so resolution/defaults can't diverge between the two. popDefault is
// the input's populationMean (numeric inputs → deviation 0 at default; the categorical smokingStatus sentinel
// → its neutral "population mix" status ⇒ mult 1). Untagged byAge stays legacy PCHIP (back-compat).
export function buildInputLanes(exogenousInputs, inputs, AGES) {
  const lanes = {};
  for (const x of exogenousInputs || []) lanes[x.id] = resolveProfile((inputs || {})[x.id], AGES, x.populationMean);
  return lanes;
}
// Per-age deviation of a direct exogenous input from its population mean, read from the resolved lane.
function inputDevLane(inputLanes, popMean, id, k) {
  const lane = inputLanes[id];
  const x = lane ? lane[k] : undefined;
...
export function simulate(MODEL, { sex, lifestyle = 1.0, interventions = {}, inputs = {}, treatments = {}, offsets = {}, operators = [], currentAge = null } = {}) {
  ...
  // ONE canonical resolved-input map (§3A) shared by the rate-term channel here AND mediators()/cause/
  // all-cause edges below — built once, passed down, so per-age input resolution can't diverge. Untagged
  // {byAge} keeps legacy PCHIP (former-smoker test byte-identical); new step/linear profiles set mode.
  const inputLanes = buildInputLanes((MODEL.bLayer && MODEL.bLayer.exogenousInputs) || [], inputs, AGES);
  const inMap = inputLanes;
  ...
  for (const op of operators || []) {
    if (op.kind === "senolytic-pulse" && NODE_IDX[op.target] !== undefined) {
      ...
      pulseOps.push({
        ...
        ages: new Set((op.ages || []).map((a) => Math.round(a))),  // snap to the DT=1 grid (§3E): a fractional
        dev: 0,                                                     // imported dosing age must still fire
  ...
  for (let k = 0; k < N_AGE; k++) {
    ...
    if (k < N_AGE - 1) {
      for (let i = 0; i < NN; i++) {
        ...
        // §3D: freeze accrues only within the HALF-OPEN window [startAge, endAge). After endAge the accrued
  ...
  const medValues = hasB ? mediators(MODEL, { sex, inputs, treatments, offsets, nodeBurdens: nodeBurdensLive, inputLanes }) : {};
  ...
  function edgeMultFor(target, k, age) {
    ...
    for (const e of list) {
      ...
      if (e.form === "nodeLogLinear") {
        ...
        m *= causeEdgeMult(e, k, age, sex, medValues, medBaseline, inputLanes, popMean);
      }
    }
    return m;
  }
  ...
  // §3A: activityFitness + sleep uShape were HOISTED here (age-invariant under scalar inputs). With
  // time-varying input lanes they are no longer constant in age, so they move INTO the per-age loop below
  // (alongside the already-per-age bmiJcurve). For a flat lane the per-age value is identical at every age ⇒
  // byte-identical to the former hoist.
  const nonBmiAllcause = allcauseEdges.filter((e) => e.form !== "bmiJcurve");
  ...
  // reaching today as given). This does NOT discount prior damage — accrued state still raises forward hazard;
  ...
  if (currentAge != null && Number.isFinite(currentAge)) {
    ...
    // extra interval; use survival[kc-1] (1 at kc=0). This makes LE_cond(AGE0) == cohort LE exactly.
    const Sc = kc === 0 ? 1 : survival[kc - 1];
    if (Sc > 0) {
      let condSum = 0;
  ...
  return { ages: AGES, T, B, hazard, decomposition, survival, LE, LE_cond, medValues };
}
...
function applyMediatorEdge(edge, inputLanes, popMean, baselineVal, K, medCtx, k) {
  ...
  const lane = inputLanes[edge.from];          // §3A: per-age input value from the canonical lane
  const x = lane ? lane[k] : undefined;
...
// (M1: the scalar `inputDev` helper was superseded by `inputDevLane` — per-age lane read — near the top.)

// Compute one Stage-2 cause-edge multiplier at age-index k / age `age`.
function causeEdgeMult(e, k, age, sex, medValues, medBaseline, inputLanes, popMean) {
  switch (e.form) {
    ...
    case "smokingCategorical": {
      ...
      const slane = inputLanes[e.input]; // categorical step lane: "never" | "former" | "current" | undefined
      const status = slane ? slane[k] : undefined;
...
function allcauseEdgeMult(e, inputLanes, popMean, k) {
  switch (e.form) {
 ...
 */
export function mediators(MODEL, { sex, inputs = {}, treatments = {}, offsets = {}, nodeBurdens = null, inputLanes = null } = {}) {
  if (sex !== "male" && sex !== "female") {
  ...
  // §3A/B/C — resolve every input channel to a per-age lane. `lanes` reuses simulate()'s canonical map when
  // passed (no divergent resolution); standalone calls build it here. `offsetLane` = personal-residual
  // trajectory per mediator (LINEAR between draws, flat outside; absent ⇒ 0). `txDoseLane` = per-age treatment
  // dose (STEP; 0 ⇒ untreated at that age ⇒ start/stop windows). A scalar/true treatment ⇒ flat full lane ⇒
  // byte-identical to the pre-M1 lifetime-constant behaviour.
  const lanes = inputLanes || buildInputLanes(b.exogenousInputs, inputs, AGES);
  const offsetLane = {};
  for (const m of b.mediators) offsetLane[m.id] = resolveProfile(offsets[m.id], AGES, 0, "linear");
  const txDoseLane = {};
  for (const tx of b.treatments) {
    const t = treatments[tx.id];
    if (t === undefined || t === false) continue;
    txDoseLane[tx.id] = resolveProfile(t === true ? 1.0 : t, AGES, 0, "step");
  }
  ...
  for (const med of order) {
    ...
    out[med.id] = AGES.map((age, k) => {
      ...
      // offset so the anchored measurement was not reproduced). resid + realized == measured at each draw.
      v += offsetLane[med.id][k];
      return v;
    });
