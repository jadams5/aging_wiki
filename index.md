...
> This is a living, AI-maintained knowledge base, not clinical guidance. Coverage is uneven: some pages are thin, unverified, or absent entirely — the absence of a page or a claim is **not** evidence of absence. Nothing here substitutes for a clinician.

This is a knowledge base on the **cellular and molecular mechanisms of human aging** — a densely cross-linked set of *atomic pages* (one per protein, pathway, compound, process, hallmark, phenotype, biomarker, study, hypothesis, and model organism) built to be operated by an AI agent, and browsable by humans. Every claim cites a primary source and every page is flagged unverified (AI-drafted) or verified against its primary-source PDF so you know how far to trust it. It doesn't duplicate UniProt, KEGG, PubChem, or PubMed — it cites them by ID and links out; the value here is the aging-specific synthesis, the cross-links, and the tracked gaps, not the raw data.
