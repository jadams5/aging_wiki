...
## Workflow
1. **Re-derive the anchor.** Independently re-pull the cause's CDC WONDER D76 2019 per-sex × age curve (or confirm the seeder's), recompute `Rmax` (age-90 anchor) + the reserve-transform burden table `B'=h/(1+h)`, and confirm they match the JSON. Correct any mismatch in the ```json block.
...
4. **Invariance check.** `node model/build-params.mjs && node model/test.mjs && node model/cli.mjs le --sex male --sex female`. Baseline LE must read 75.815/80.862 for re-bucketing (Op A/B) and unwired Op C. If a wired change legitimately moved an LE/ΔLE target, confirm the new value is physiologically sane, then re-baseline the affected `test.mjs` targets **and** the `build-app.mjs` self-checks **and** the doc references — don't assume the test is wrong.
