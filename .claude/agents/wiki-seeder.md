---
name: wiki-seeder
description: Drafts new entity pages for the aging_wiki from canonical databases + training knowledge. Use for filling out planned-but-not-yet-seeded entities — surfaced by inbound-count discovery (`lint-pass` § Step 3), ad-hoc as new studies/questions arise, or from the curated proactive list in `gaps/planned-coverage.md`. Reads canonical DBs (UniProt, KEGG, Reactome, PubChem, GenAge) via WebFetch, uses the local paper archive for citation discovery, runs a recency literature search (PubMed + Europe PMC, last 5–7 yr) for high-velocity entity types (compound, intervention, biomarker, hypothesis) so the page is not anchored to training-era sources, drafts following the schema in CLAUDE.md, ships with `verified: false` + ⚠️ banner. Pairs with the `wiki-verifier` subagent which flips the flag after PDF cross-check.
