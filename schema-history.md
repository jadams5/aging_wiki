...
## R52 — clinical-stage: approved-ex-us (2026-06-09)
- **Precedent:** [[molecules/compounds/pdrn]] (polydeoxyribonucleotide/polynucleotide injectables — CE-marked + Korea MFDS-approved for aesthetic/wound-healing indications, not FDA-approved). Surfaced during the injectable-skin-boosters build-out (2026-06-09); both the seeder and the verifier independently escalated that the `clinical-stage:` enum had no value for "approved by a stringent non-US regulator but not FDA," forcing a `phase-2` surrogate.
- **New enum value:** `approved-ex-us` added to the `type: compound` `clinical-stage:` enum (`preclinical | phase-1 | phase-2 | phase-3 | approved-ex-us | fda-approved | supplement`). By the existing convention (aging-context `clinical-stage` on intervention class pages, CLAUDE.md § druggability conventions) it also applies to `clinical-stage:` on `type: intervention` class pages.
- **Semantics:** denotes real-world marketing approval for the relevant aging/aesthetic indication by EMA/EU (incl. CE marking), UK MHRA, Japan PMDA, South Korea MFDS, or comparable stringent regulator, **without** FDA approval. Body must name the jurisdiction(s). When approved in BOTH the US and elsewhere → use `fda-approved` (body notes the rest); `approved-ex-us` is specifically the *US-gap* case.
- **Rationale (user, 2026-06-09):** ex-US approval is **not** a weaker signal than FDA approval, and can be a stronger or earlier one. Motivating cases: PDRN injectables (mainstream in EU/Korea aesthetics, US-investigational) and next-generation sunscreen UV filters (bemotrizinol, bisoctrizole, etc.) that are EMA/global-approved but stuck in the US OTC-monograph backlog — there, "not FDA-approved" reflects US regulatory *lag* and FDA's drug-vs-cosmetic classification, not evidence weakness. The wiki needs to record that distinction rather than collapse it into a misleadingly-low `phase-N`.
- **Applied:** [[molecules/compounds/pdrn]] `clinical-stage: phase-2 → approved-ex-us`; body regulatory note rewritten accordingly. No other existing page currently needs the value (biostimulatory-fillers PLLA/CaHA are FDA-approved; PCL/PDO are CE-only but live on an `implemented` intervention class page).
- **Lint:** accept `approved-ex-us` as a valid `clinical-stage:` on compound + intervention pages. No cadence/required-field change.
- **CLAUDE.md status:** enum line + a dedicated "Ex-US regulatory-approval convention (R52)" paragraph added to the shared druggability-conventions section in this pass (not deferred).

## R53 — type:exposure (2026-06-11)
- **New page type for modifiable environmental/behavioral risk factors** that are neither a compound, an intervention, nor a phenotype: tobacco smoking, alcohol, ambient air pollution, and similar exposures. Lives in a new top-level `exposures/` directory.
- **Rationale (user, 2026-06-11):** surfaced while wiring node-description tooltips + wiki links into the aging-simulator (`viz/aging-simulator.html`). The simulator's exogenous-input nodes (smoking, alcohol, PM2.5 air pollution) had **zero** wiki coverage and no schema home — they are exposures the individual is *subjected to / chooses*, not drugs taken (`type: compound`), not deliberate regimens (`type: intervention`, which frames the *modifiable lever* — smoking-*cessation* — awkwardly for an ambient exposure like PM2.5), and not conditions of the organism (`type: phenotype`). The user chose to formalize a dedicated type rather than force a half-fit or leave them link-less. (The disease nodes from the same pass — COPD, CKD, hypertension, obesity — fit `type: phenotype` cleanly and were seeded there, not here.)
- **Frontmatter:** `exposure-class` (behavioral | environmental | occupational | dietary), `agent` (the harmful constituent(s)), `measure` (how it's quantified — pack-years, g/day ethanol, µg/m³ PM2.5), `modifiable` (yes | no | partial), `dose-response` (monotonic | j-curve | threshold | sublinear | supralinear), `target-hallmarks`, `target-pathways` (opt), `downstream-phenotypes`, `mechanisms`, `human-evidence-level` (mirrors compound), `reversibility` (free-text; cessation effect + time-course), `mendelian-randomization` (reuses the protein/biomarker enum — smoking/alcohol are MR-instrumented via CHRNA3/ADH1B/ALDH2), `literature-checked-through` (required, 18mo cadence — risk-factor epidemiology moves but slower than drug pipelines).
- **Verification discipline:** atomic content (originates relative risks, dose-response curves, MR estimates) → carries the full four-field `verified` block like compound/biomarker/phenotype.
- **Initial set (3):** [[exposures/smoking]], [[exposures/alcohol]], [[exposures/air-pollution]] (seeded `verified: false`). Distinct from `interventions/dietary/sodium-restriction` etc., which remain interventions (a chosen reduction regimen); a *dietary component* viewed as harmful exposure (e.g. dietary saturated fat) stays on its compound page (`molecules/compounds/palmitic-acid`) unless it earns its own synthesis page.
- **Lint:** `exposures/` added to the atomic-content directories; `type: exposure` added to the verified-required + literature-recency-required type lists (18mo).

## R54 — type:phenotype literature-checked-through (2026-06-11)
- **Added the optional `literature-checked-through:` field to `type: phenotype`** (18mo cadence, optional — populate for fast-moving phenotypes, leave null for slow-turnover descriptive ones). Mirrors the existing optional treatment on protein/pathway/process/hallmark/microbe/model-organism.
- **Rationale:** during the R53 seed batch (COPD, CKD, hypertension, obesity — all phenotype pages with active intervention + epidemiology literature), three of the four `wiki-seeder` agents independently flagged that they had run the R25 recency search but had nowhere in the phenotype frontmatter to record `literature-checked-through:`, so they omitted it (correctly, per the then-current schema). The convergent signal from independent agents made the gap clear: phenotypes like CKD (SGLT2i/finerenone), hypertension (SPRINT-era targets), and obesity (GLP-1/SURMOUNT) have fast-moving therapeutic landscapes where a recency stamp is as load-bearing as on a compound page.
- **Scope:** optional (not required) — most phenotypes are slow-turnover descriptive entities (e.g. hair-greying, ear-nose-enlargement) where the field would be noise. Lint flags it only when *populated* and stale, same as the other optional types.
- **Applied:** the four R53 phenotype pages (copd, chronic-kidney-disease, hypertension, obesity) backfilled with `literature-checked-through: 2026-06-11` (each had run the recency search at seed time).

## R55 — biomarker modality: vital-sign (2026-06-11)
- **Added `vital-sign` to the `type: biomarker` `modality:` enum.** For single-parameter resting physiological measures — resting heart rate, resting blood pressure, heart-rate variability, respiratory rate, body temperature — that index aging/mortality risk but are neither molecular clocks nor performance tests.
- **Rationale (user, 2026-06-11):** seeding `biomarkers/resting-heart-rate-biomarker.md` (to ground the aging-simulator's resting-HR node) exposed that no existing modality fit: it is not a `composite-clinical` score (single parameter, not a multi-feature index like PhenoAge), not `physical-performance` (no performance/exertion test — it's a resting vital sign), and not any of the omics/imaging/telomere modalities. The seeder used `physical-performance` under protest and flagged the misfit; the verifier confirmed it and (correctly) did not invent an enum value. `vital-sign` was chosen over the narrower `autonomic` because it also cleanly homes resting BP, respiratory rate, and temperature (not all of which are purely autonomic).
- **Lint:** for `modality: vital-sign`, the clock-feature fields (`n-cpgs-or-features`, `training-cohort`, `model-architecture`, `test-retest-icc`) are structurally null/N-A — lint must NOT flag their absence (same precedent as the `svm` + null `n-cpgs-or-features` carve-out).
- **Applied:** [[biomarkers/resting-heart-rate-biomarker]] `modality: physical-performance → vital-sign`; body note explaining the prior misfit replaced with a note recording the R55 classification.

## R56 — biomarker training-target: exposure (2026-06-14)
- **Added `exposure` to the `type: biomarker` `training-target:` enum** (`chronological-age | mortality | morbidity | pace | exposure`). For DNA-methylation / omic predictors trained to estimate an **external exposure or behavioral quantity** — alcohol intake, smoking intensity, BMI, dietary pattern — rather than a biological age, mortality, morbidity, or pace-of-aging outcome.
- **Rationale (user, 2026-06-14):** seeding [[biomarkers/dnam-alcohol-consumption]] (the 144-CpG DNAm-Alc predictor behind TruDiagnostic's "Alcohol epigenetic biomarker") exposed that no enum value fit — it predicts grams/AUDIT of alcohol consumed, not an age or vital outcome. The seeder used `consumption` under protest and flagged the gap; the verifier correctly left it for adjudication rather than inventing a value. `exposure` was chosen over the narrower `consumption` because it generalizes to the whole class of DNAm exposure-predictors (the same elastic-net/LASSO-on-CpGs architecture is used for smoking pack-years, BMI, and diet scores), several of which are likely future seeds.
- **Note:** these exposure-predictor biomarkers are distinct from `type: exposure` pages (R53) — the exposure page is the *risk factor itself* (e.g. [[exposures/alcohol]]); an `exposure`-target biomarker is the *methylation readout that estimates* that exposure. The two cross-link (the exposure page links to its DNAm readout, and vice versa).
- **Applied:** [[biomarkers/dnam-alcohol-consumption]] `training-target: consumption → exposure`.
