...
Most wikis are written for people and occasionally read by machines. This one is the other way around: it's designed so an LLM agent can use it as a knowledge base to better serve people — reading broadly before answering, extending it with new findings, and verifying its own claims against primary sources. The result happens to be a pretty good wiki for humans too.
